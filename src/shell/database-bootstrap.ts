...
export function initDatabase(): DatabaseBootstrapResult {
  ...
  // Failures are logged, never swallowed (Q-02): without these, integrity
  // (foreign_keys) and concurrency (WAL) guarantees silently vanish.
  const pragma = (sql: string) =>
    dbAdapter.execute(sql).catch((err: unknown) => {
      console.warn(
        `[DatabaseBootstrap] PRAGMA failed (${sql}):`,
  ...
  const initSchemaPromise = (async () => {
    try {
      ...
      console.error(
        "[database-bootstrap] Schema init or migration error:",
        err,
      );
    }
  })();
 ...
 */
async function seedOperationalDefaults(
  db: SQLiteDatabaseAdapter,
): Promise<void> {
  // 1. Seed Initial Feed Posts
  const feedCount = await db
    .query<{ count: number }>(`SELECT COUNT(*) as count FROM shell_feed`)
    .catch(() => [{ count: 1 }]);
  ...
  const prodCount = await db
    .query<{ count: number }>(`SELECT COUNT(*) as count FROM commerce_products`)
    .catch(() => [{ count: 1 }]);
  ...
  const portCount = await db
    .query<{ count: number }>(`SELECT COUNT(*) as count FROM portfolio_items`)
    .catch(() => [{ count: 1 }]);
  ...
  const spaceCount = await db
    .query<{ count: number }>(`SELECT COUNT(*) as count FROM spaces`)
    .catch(() => [{ count: 1 }]);
  ...
  const beamCount = await db
    .query<{ count: number }>(`SELECT COUNT(*) as count FROM beam_channels`)
    .catch(() => [{ count: 1 }]);
  ...
  const bookCount = await db
    .query<{ count: number }>(`SELECT COUNT(*) as count FROM booking_slots`)
    .catch(() => [{ count: 1 }]);
  ...
  const impCount = await db
    .query<{ count: number }>(`SELECT COUNT(*) as count FROM imperia_proposals`)
    .catch(() => [{ count: 1 }]);
  ...
  const solCount = await db
    .query<{ count: number }>(
      `SELECT COUNT(*) as count FROM solidarity_campaigns`,
    )
    .catch(() => [{ count: 1 }]);
