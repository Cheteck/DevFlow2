/**
 * pnpm check:integrity [--strict] [--format=json|sarif] — bypass-analysis report over the repo.
 *
...
async function main(): Promise<void> {
  ...
  const format = formatArg ? formatArg.split("=")[1]?.toLowerCase() : null;
  ...
  if (format === "json") {
    console.log(IntegrityConformanceSuite.toJSON(findings));
  } else if (format === "sarif") {
    console.log(IntegrityConformanceSuite.toSARIF(findings));
  } else {
    console.log(
    ...
    for (const f of findings) {
      ...
      console.log(
        `[check:integrity] [${level}] ${f.ruleId} ${f.file}:${f.line}:${f.col} — ${f.excerpt}`,
      );
    }
  ...
  if (strict && summary.errors > 0 && format) {
    process.exit(1);
  }
}
