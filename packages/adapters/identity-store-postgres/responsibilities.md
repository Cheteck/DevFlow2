# Responsabilites — adapter identity-store-postgres

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-identity-store-postgres` |
| Version | `0.1.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-identity-store` : Profils/identites sur PostgreSQL (+ migrations incluses).
...
Depends de `ports-identity-store` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
