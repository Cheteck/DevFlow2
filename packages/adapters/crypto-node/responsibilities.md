# Responsabilites — adapter crypto-node

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-crypto-node` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-crypto` : Crypto via node:crypto (hash, randomBytes, sign) — runtime Node.
...
Depends de `ports-crypto` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
