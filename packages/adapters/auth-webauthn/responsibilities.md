# Responsabilites — adapter auth-webauthn

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-auth-webauthn` |
| Version | `0.1.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports (auth)` : Authentification WebAuthn/passkeys (challenge, attestation, assertion).
...
Depends de `ports (auth)` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
