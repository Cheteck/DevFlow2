# Responsabilites — adapter logger-winston

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-logger-winston` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-logging` : Logs via Winston (transports multiples).
...
Depends de `ports-logging` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
