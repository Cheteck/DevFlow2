# Responsabilites — adapter clock-fake

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-clock-fake` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-clock` : Horloge controllable pour tests (avance manuelle du temps, determinisme).
...
Depends de `ports-clock` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
