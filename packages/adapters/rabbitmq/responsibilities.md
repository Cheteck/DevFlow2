# Responsabilites — adapter rabbitmq

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-rabbitmq` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-pubsub` : Pub/Sub via RabbitMQ (exchanges, queues).
...
Depends de `ports-pubsub` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
