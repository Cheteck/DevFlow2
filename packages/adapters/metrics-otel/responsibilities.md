# Responsabilites — adapter metrics-otel

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-metrics-otel` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-metrics` : Metriques via OpenTelemetry.
...
Depends de `ports-metrics` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
