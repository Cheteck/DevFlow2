# Responsabilites — adapter credential-store-sqlite

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-credential-store-sqlite` |
| Version | `0.1.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-credential-store` : Persistance credentials sur SQLite (dev/edge).
...
Depends de `ports-credential-store` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
