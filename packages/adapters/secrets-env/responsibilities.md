# Responsabilites — adapter secrets-env

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-secrets-env` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-secrets` : Secrets lus depuis environnement (dev ; prod = vault a venir).
...
Depends de `ports-secrets` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
