# Responsabilites — adapter featureflags-launchdarkly

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-featureflags-launchdarkly` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-feature-flags` : Flags via LaunchDarkly (SDK, streaming).
...
Depends de `ports-feature-flags` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
