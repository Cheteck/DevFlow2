# Responsabilites — adapter config-env

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-config-env` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-config` : Lecture de configuration depuis variables environnement + fichiers .env.
...
Depends de `ports-config` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
