# Responsabilites — adapter http-fetch

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-http-fetch` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-http` : Client HTTP base sur fetch natif.
...
Depends de `ports-http` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
