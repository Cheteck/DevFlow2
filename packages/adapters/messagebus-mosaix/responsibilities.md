# Responsabilites — adapter messagebus-mosaix

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-messagebus-mosaix` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-message-bus` : Bus in-memory MosaiX (dev/test, sans broker).
...
Depends de `ports-message-bus` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
