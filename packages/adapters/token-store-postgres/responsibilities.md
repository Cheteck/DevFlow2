# Responsabilites — adapter token-store-postgres

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-token-store-postgres` |
| Version | `0.1.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-token-store` : Tokens sur PostgreSQL.
...
Depends de `ports-token-store` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
