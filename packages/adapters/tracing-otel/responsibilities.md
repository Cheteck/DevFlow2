# Responsabilites — adapter tracing-otel

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-tracing-otel` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-tracing` : Traces via OpenTelemetry (spans, export OTLP).
...
Depends de `ports-tracing` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
