# Responsabilites — adapter session-store-redis

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-session-store-redis` |
| Version | `0.1.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-session-store` : Sessions sur Redis (TTL, revocation).
...
Depends de `ports-session-store` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
