# Responsabilites — adapter id-ulid

| Champ | Valeur |
|---|---|
| Nom | `@mosaix/adapter-id-ulid` |
| Version | `1.0.0` |
| Couche | Adapter concret (Layer 3) |

## Raison d'etre

Implementer `ports-id` : Generation ULID (triable lexicographiquement).
...
Depends de `ports-id` (+ SDK externe). Selectionne via composition (`config/compositions/`, DatabaseModule/resolver) avec fallback in-memory conditionnel.
